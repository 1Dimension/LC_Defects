...
int main(int argc,char *argv[]) 
{
  ...
  //设定物理参数
  double A = - 0.172 * 1e6, B = -2.12 * 1e6, C = 1.73*1e6, L1 = 4 * 1e-11, R0 = 5 * 1e-7, W = 100;
  double D0 = 3 * R0;  ///球形粒子球心的距离
  ...
  Basis_init(64,64,3,1001,1001,2);
  ...
  for (int i = 0; i < I; i++) 
  {
    for (int j = 0; j < J; j++) 
    {
      ...
      for (int n = 0; n < 5; n++)
	{
	  // Qijk[n * Point + i * J * K + j * K + k]  += q_far[n];
	  Qijk[n * Point + i * J * K + j * K + k] = Qijk[n * Point + i * J * K + j * K + k] + q_far[n];
	}
      }
    }
  }
  ...
  */
  ...
  sprintf(fname,"%s/Drawpoint/%s%s_t_%.2f_R_%.2f_D_%.2f_I_%d_J_%d_K_%d_eta_%.1f_L21_%+.1f_s0_%.2f_%s_point.txt",DIR,func_type,boundary,landau_t,Rad, D, I,J,K,eta,L21,beig,argv[1]);
  ...
  for (int k = 0; k < K; k++) 
  {
	...
	*/
	...
	// vec[1][i3] = 0;
	fprintf(fp,"%.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.10e %.6e\n",x,y,z,eg[i1],eg[i2],eg[i3],vec[0][i3],vec[1][i3],vec[2][i3],beta,fbulk[i * J * K + j * K + k], eg[i3] - eg[i2], q[0], QL2[i * J * K + j * K + k]);
	// ix++;
      }
    }
  }
